...
namespace Dothan.Helpers
{
    ...
    public static class FileName
    {
        ...
        /// <summary>
        /// 判断当前文件是否存在，如果存在，则进行递增判断，直至找到一个可用的文件为止；
        /// </summary>
        /// <param name="file">带判断文件；</param>
        /// <param name="maxIndex">地边查找的最大索引值，默认为1024；</param>
        /// <returns>获取到的可用文件；</returns>
        public static String IncreateFileName(String file, int maxIndex = 1024)
        {
            ...
            {
                currFile = Path.Combine(filePath, fileName + i + ext);
